...
export default function IncidentDetail({ incident, onClose, onReplay }) {
  ...
  const { acknowledge, resolve, explain, ask, applyFix } = useIncidentStore();
      ...
      <Card style={{ padding: 30 }} glow={sev.glow}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 22 }}>
          <div style={{ flex: 1, paddingRight: 16 }}>
            <div style={{ display: "flex", gap: 7, flexWrap: "wrap", marginBottom: 10 }}>
              <Badge color={sev.color} glow={sev.glow}>{sev.label}</Badge>
              <Badge color={st.color}>{st.label}</Badge>
              <span style={{ fontSize: 12, color: COLORS.textMuted }}>{timeAgo(incident.started_at)}</span>
            </div>
            <h2 style={{ fontSize: 17, fontWeight: 500, color: COLORS.textPrimary, lineHeight: 1.45 }}>
              {incident.title}
            </h2>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 22 }}>
          {[
            ...
            { label: "Containers", value: incident.affected_containers?.length || 0, color: COLORS.orange },
          ].map(s => (
            <div key={s.label} style={{ background: COLORS.bg, borderRadius: 9, padding: "13px 16px",
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 11, color: COLORS.textMuted, fontFamily: "'Space Mono', monospace",
            textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 10 }}>🤖 AI Explanation</div>
          <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
            {MODES.map(m => (
              <button key={m.key} onClick={() => handleExplain(m.key)} style={{
          ...
          <div style={{ background: COLORS.bg, borderRadius: 10, padding: "14px 16px",
            border: `1px solid ${COLORS.borderLight}`, minHeight: 80,
            fontSize: 13.5, color: COLORS.textSecondary, lineHeight: 1.7,
            display: "flex", alignItems: loadingMode === mode ? "center" : "flex-start",
            justifyContent: loadingMode === mode ? "center" : "flex-start" }}>
            {loadingMode === mode ? <Spinner /> : (aiContent[mode] || "Click a mode above to generate explanation")}
          </div>
        </div>
        ...
        {incident.ai_fix_suggestion && (
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 11, color: COLORS.textMuted, fontFamily: "'Space Mono', monospace",
              textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 10 }}>🔧 Suggested Fix</div>
            <div style={{ background: `${COLORS.green}10`, border: `1px solid ${COLORS.green}30`,
              borderRadius: 10, padding: "12px 16px", fontSize: 13,
              color: COLORS.green, lineHeight: 1.6, marginBottom: 10 }}>
              {incident.ai_fix_suggestion}
            </div>
            {incident.ai_auto_fix_script && !incident.ai_fix_applied && (
              <div style={{ display: "flex", gap: 8 }}>
                {!fixResult ? (
                  <Button small onClick={() => handleFix(false)} disabled={actionLoading}>
                    {actionLoading ? <Spinner size={14} /> : "Preview Script"}
                  </Button>
                ) : fixResult.script_preview ? (
                  <>
                    <div style={{ background: COLORS.bg, borderRadius: 8, padding: "10px 14px",
                      fontFamily: "'Space Mono', monospace", fontSize: 11,
                      color: COLORS.textSecondary, flex: 1, whiteSpace: "pre-wrap" }}>
                      {fixResult.script_preview}
                    </div>
                    <Button small primary onClick={() => handleFix(true)} disabled={actionLoading}>
                      {actionLoading ? <Spinner size={14} /> : "Apply Fix"}
                    </Button>
                  </>
                ) : fixResult.success ? (
                  <Alert type="success">✓ Fix applied successfully</Alert>
                ) : null}
              </div>
            )}
            {incident.ai_fix_applied && (
              <Alert type="success">✓ AI fix was applied at {new Date(incident.ai_fix_applied_at).toLocaleString()}</Alert>
            )}
          </div>
        )}
        ...
        <div style={{ marginBottom: 22 }}>
          <div style={{ fontSize: 11, color: COLORS.textMuted, fontFamily: "'Space Mono', monospace",
            textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 10 }}>💬 Ask AI</div>
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <Button small primary onClick={handleAsk} disabled={askLoading || !question.trim()}>
              {askLoading ? <Spinner size={14} /> : "Ask"}
            </Button>
          </div>
          {answer && (
            <div style={{ marginTop: 10, background: COLORS.bg, borderRadius: 8,
              padding: "12px 14px", fontSize: 13, color: COLORS.textSecondary,
              lineHeight: 1.65, border: `1px solid ${COLORS.border}`,
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <Button primary onClick={() => { onClose(); onReplay(incident); }} style={{ flex: 1 }}>
            ⏮ Incident Replay
          </Button>
          {incident.status === "open" && (
            <Button onClick={() => acknowledge(incident.id)} style={{ flex: 1 }}>
              ✓ Acknowledge
            </Button>
          )}
