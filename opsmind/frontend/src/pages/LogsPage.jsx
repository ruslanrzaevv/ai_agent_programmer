...
import { Button, Card, LiveDot, SectionTitle } from "../components/ui";
...
export default function LogsPage() {
  ...
  const { logs, loading, filter, paused, fetch, setFilter, setPaused, clear } = useLogStore();
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 600, color: COLORS.textPrimary,
            letterSpacing: "-0.02em", marginBottom: 4 }}>Live Logs</h1>
          <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
            {connected && !paused
              ? <><LiveDot /><span style={{ color: COLORS.green }}>Streaming in real-time</span></>
              : <span style={{ color: COLORS.textMuted }}>⏸ {paused ? "Paused" : "Connecting..."}</span>
            }
            <span style={{ color: COLORS.textMuted }}>· {filtered.length} entries</span>
          </div>
        </div>
      ...
      <div style={{ display: "flex", gap: 7, marginBottom: 14 }}>
        {LEVELS.map(l => (
          <button key={l} onClick={() => setFilter(l)} style={{
            ...
            background: filter === l ? `${LOG_COLORS[l] || COLORS.accent}20` : "transparent",
            color: filter === l ? (LOG_COLORS[l] || COLORS.accent) : COLORS.textMuted,
            border: `1px solid ${filter === l ? (LOG_COLORS[l] || COLORS.accent) + "50" : COLORS.border}`,
          }}>{l}</button>
        ))}
      </div>

      {/* Log stream */}
      <Card style={{ maxHeight: "calc(100vh - 260px)", overflow: "auto", padding: "4px 0" }}>
        {filtered.length === 0 ? (
          ...
          filtered.map((log, i) => (
            <div key={log.id || i} style={{
              ...
              animation: i === 0 ? "fade-in 0.25s ease" : "none",
              ...
              <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 10, fontWeight: 700,
                color: LOG_COLORS[log.level] || COLORS.textMuted, textTransform: "uppercase",
                flexShrink: 0, width: 60, paddingTop: 1 }}>
                {log.level}
              </span>
              <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 10,
                color: COLORS.accent, flexShrink: 0, width: 100, paddingTop: 1,
                overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {log.container_name || log.service_name || "—"}
              </span>
              <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 11, flex: 1,
                color: ["error","critical"].includes(log.level) ? COLORS.textPrimary : COLORS.textSecondary,
